...
type OctantTransform(xx:DistanceType, xy:DistanceType, yx:DistanceType, yy:DistanceType) =
    member _.XX = xx
    member _.XY = xy
    member _.YX = yx
    member _.YY = yy
...
let OctantTransforms =
    [|
        new OctantTransform(  1s,  0s,  0s,  1s ) // 0 E-NE
        new OctantTransform(  0s,  1s,  1s,  0s ) // 1 NE-N
        new OctantTransform(  0s, -1s,  1s,  0s ) // 2 N-NW
        new OctantTransform( -1s,  0s,  0s,  1s ) // 3 NW-W
        new OctantTransform( -1s,  0s,  0s, -1s ) // 4 W-SW
        new OctantTransform(  0s, -1s, -1s,  0s ) // 5 SW-S
        new OctantTransform(  0s,  1s, -1s,  0s ) // 6 S-SE
...
let computeVisibility_Shadowcast1 (viewerLocation:Location) (grid:Location[]) (forms:Map<Location,FormComponent[]>) (viewRadius:DistanceType) =
    ...
    let rec castLight (startColumn:DistanceType) (lvs:float) (rightViewSlope:float) (txfrm:OctantTransform) = 
            ...
            while yc >= 0s do
                ...
                | true -> 
                    ...
                    | false ->
                        ...
                        | true -> 
                            yc <- -1s // Because we want to abort the loop
                        | false ->
                            let curBlocked = 
                                forms.Item l 
                            ...
                            | false ->
                                ...
                                | true ->
                                    if (leftBlockSlope <= leftViewSlope) then
                                        castLight (currentCol + 1s) leftViewSlope leftBlockSlope txfrm 
                                    prevWasBlocked <- true
                                    savedRightSlope <- rightBlockSlope
                            viewableMap <- viewableMap.Add(l,forms.Item(l)) // Array.append viewableMap [|l|]
                yc <- yc - 1s
            currentCol <- currentCol + 1s
    OctantTransforms |> Array.iter (fun ot -> castLight 1s 1.0 0.0 ot) // Can't Parallel
    viewableMap


(*
let ComputeVisibility_original (viewerLocation:Location) (grid:Location[]) (forms:Map<Location,FormComponent[]>) (viewRadius:int) =
