...
let log_ComponentUpdate (l:string[]) (result:string) (system:string) (step:string) (eid:EntityID) (cid:ComponentID) (oo:'a option) = 
    match oo with 
    | None   -> 
        sprintf "%-3s | %-20s -> %-30s #%7i.%-7i" result system step eid.ToUint32 cid.ToUint32
    | Some s when s.GetType() = typeof<string> -> 
        sprintf "%-3s | %-20s -> %-30s #%7i.%-7i : %s" result system step eid.ToUint32 cid.ToUint32 (s.ToString())
    | Some o -> 
        sprintf "%-3s | %-20s -> %-30s #%7i.%-7i : %A" result system step eid.ToUint32 cid.ToUint32 o
    |> log l
